...
public class Des {
    ...
    public static String decrypt(String cihperText) {
        int i;
        String key = "AABB09182736CCDD";
        // get round keys
        String keys[] = getKeys(key);
        ...
        // initial permutation
        cihperText = permutation(IP, cihperText);

        // 16-rounds
        for (i = 15; i > -1; i--) {
            cihperText
                    = round(cihperText, keys[i], 15 - i);
        }

        // 32-bit swap
        cihperText = cihperText.substring(8, 16)
                + cihperText.substring(0, 8);
        cihperText = permutation(IP1, cihperText);
        return cihperText;
    }
